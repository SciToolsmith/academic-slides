...
暂存重建是可复现性和客户包边界检查，不是新的视觉编辑循环。若内部已完成的 <code>--render</code> 预览之后，MJS、交付资产和运行时均未变化，则记录暂存 parity 与安全校验即可；不要为了“再确认一次”重复渲染完整 PPT 或 Word。将构建与暂存的耗时指标留在内部 QA 目录，不放进客户包。
