...
1. 校验 `project-config.json`、profile 对应的 `figures.manifest.json` 或 `paper-assets.json`、`paper-index.json`、`evidence-index.json` 和 `deck-spec.json` 的 JSON Schema。
2. 执行跨文件与跨字段语义检查。
...
- 贡献能回指前面展示的证据。
- 页面之间有自然过渡，没有重复论证。
- 答辩类附录只包含重要防守材料且不干扰主线；文献组会默认无可见附录，学生结束页必须是最后一张。
...
出现以下问题不得正式交付：文件无法打开、源文件缺失、核心数字或入选公式错误、证据引用断裂、组会核心发现无实际源证据、公式中心论文零核心公式、严重溢出遮挡、备注或页面大量缺失、校徽机构错误、敏感信息泄露。
