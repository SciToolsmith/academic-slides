...
6. 先看全稿联系表和节奏，再对核心结果、公式、表格、复杂科研图、自由画布、自动风险页、封面和结束页做全尺寸复核；简单过渡页不必逐张像素级检查。检查 Word 标题、分页、逐页编号和讲稿顺序；来源继续在 PPT 备注中核对。
...
- 上一条只检查页面是否有第一落点；组会核心发现仍必须实际渲染源证据，`text_emphasis` 单独不能满足 `group_meeting_v1`；
